...
template <typename T>
__global__ void MulConvFCForward(const int nthreads, 
...
template <typename T>
__global__ void MulConvFCBackward(const int nthreads, const T* input_grad,
    const T* input1, const T* input2,
    const int pixels, T* output_grad1, T* output_grad2) {
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    const int idx = index / pixels;
    // first for X1
    output_grad1[index] = input_grad[index] * input2[idx];
...
template<>
bool MulConvFCOp<float, CUDAContext>::RunOnDevice() {
  ...
  MulConvFCForward<float><<<CAFFE_GET_BLOCKS(output_size), CAFFE_CUDA_NUM_THREADS,
...
template<>
bool MulConvFCGradientOp<float, CUDAContext>::RunOnDevice() {
  ...
  MulConvFCBackward<float><<<CAFFE_GET_BLOCKS(output_size),
...
REGISTER_CUDA_OPERATOR(MulConvFC,
                       MulConvFCOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(MulConvFCGradient,
                       MulConvFCGradientOp<float, CUDAContext>);
} // namespace caffe2