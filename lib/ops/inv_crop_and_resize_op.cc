...
REGISTER_CPU_OPERATOR(InvCropAndResize, InvCropAndResizeOp<float, CPUContext>);
REGISTER_CPU_OPERATOR(InvCropAndResizeGradient, InvCropAndResizeGradientOp<float, CPUContext>);

OPERATOR_SCHEMA(InvCropAndResize)
    .NumInputs(3)
    .NumOutputs(1)
    .SetDoc(R"DOC(
Inverse of crop and resize Operation.
)DOC")
    .Arg(
        "spatial_scale",
...
OPERATOR_SCHEMA(InvCropAndResizeGradient)
    .NumInputs(4)
    .NumOutputs(1)
    .Input(
        0,
...
class GetInvCropAndResizeGradient : public GradientMakerBase {
  using GradientMakerBase::GradientMakerBase;
  vector<OperatorDef> GetGradientDefs() override {
    return SingleGradientDef(
        "InvCropAndResizeGradient",
        "",
...
REGISTER_GRADIENT(InvCropAndResize, GetInvCropAndResizeGradient);
