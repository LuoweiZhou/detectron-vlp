...
__global__ void ComputeOverlapping(const int nthreads,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    if (idA < idB) {
      const int iidA = static_cast<int>(indices[idA]) * 13;
      const int iidB = static_cast<int>(indices[idB]) * 13;
...
__global__ void NMSOnlyForward(const int nthreads,
...
__global__ void CopyBoxes(const int nthreads,
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    const int bid = i * 13;
    const int oid = i * 12;
    _copy(7, boxes + bid, output_boxes + oid);
    _copy(5, boxes + bid + 8, output_boxes + oid + 7);
  }
}

__global__ void NMSOnlyReduceBoxes(const int nthreads,
                          const float* boxes,
                          const int* index,
                          float* output_boxes) {
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
...
template<>
bool NMSOnlyOp<float, CUDAContext>::RunOnDevice() {
  auto& boxes = Input(0);
  ...
  context_.Copy<float, CUDAContext, CUDAContext>(num_total * 13, boxes_pointer, 
                                                middle_pointer);
  ...
  NMSOnlyForward<<<CAFFE_GET_BLOCKS(1), CAFFE_CUDA_NUM_THREADS,
                  0, context_.cuda_stream()>>>(1,
                                              overlaps_pointer,
                                              indices_pointer,
                                              num_total,
  ...
  out_boxes->Resize(num_reduced, 12);
  float* out_boxes_pointer = out_boxes->mutable_data<float>();

  NMSOnlyReduceBoxes<<<CAFFE_GET_BLOCKS(num_reduced), CAFFE_CUDA_NUM_THREADS,
...
REGISTER_CUDA_OPERATOR(NMSOnly,
                       NMSOnlyOp<float, CUDAContext>);
} // namespace caffe2