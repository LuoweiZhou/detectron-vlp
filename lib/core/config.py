...
def get_output_dir(training=True, weights=None):
    ...
    outdir = osp.join(__C.OUTPUT_DIR, tag, dataset, __C.CFG_FILE, __C.CFG_OPTS)
    if not training and weights:
        basename_weights = os.path.basename(weights)
        basename = os.path.splitext(basename_weights)[0]
        outdir = osp.join(outdir, basename)
