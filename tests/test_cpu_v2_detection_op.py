...
class GPUDetectionOpTest(unittest.TestCase):
    ...
    def _run_boxes_only_op_cpu(self, box_pred, anchor,
                                    yi, yv, im_info, lvl):
        op_name = 'BoxesOnly'
    ...
    def _run_class_based_boxes_only_op_cpu(self, stats, boxes, class_id):
    ...
    def _run_class_based_boxes_combined_op_cpu(self, stats, boxes, num_classes):
        op_name = 'ClassBasedBoxesCombined'
        blobs_in = ['stats', 'boxes']
        values_in = [stats, boxes]
        blobs_out = []
        for cls in range(num_classes):
            blobs_out.append('blobs_cls_%d' % cls)
        args = {}
        return self._run_general_op_cpu(op_name, blobs_in, values_in, blobs_out, **args) 
    ...
    def _run_reduce_boxes_only_op_cpu(self, boxes):
    ...
    def _run_reduce_sum_op_cpu(self, values_in):
    ...
    def _run_distribute_op_cpu(self, boxes, stats, num_levels):
        op_name = 'Distribute'
        blobs_in = ['boxes', 'stats']
        values_in = [boxes, stats]
        blobs_out = ['rois', 'frois']
        for i in range(num_levels):
            blobs_out.append('rois_%02d' % (i+3))
        args = {'k_min': 3, 'k_max': 7}
        return self._run_general_op_cpu(op_name, blobs_in, values_in, blobs_out, **args) 
    ...
    def _run_get_logits_op_gpu(self, cls_preds, frois):
        ...
        for i, cp in enumerate(cls_preds):
            ...
            values_in.append(cp)
        blobs_out = ['feats']
        args = {'num_feats': 80, 'k_min': 3, 'all_dim': False}
        return self._run_general_op_gpu(op_name, blobs_in, values_in, blobs_out, **args) 

    def _run_cpu_testing(self, cls_preds, cls_probs, bbox_preds, anchors, im_info):
        # run select top n op
        num_images = 1
        num_classes = 81
        ...
        for cr, cp, bp, ac in zip(cls_preds, cls_probs, bbox_preds, anchors):
            picked = self._run_select_top_n_single_op_cpu(cp, 1000, 0)
            yi = picked[0]
            yv = picked[1]

            bfs = self._run_boxes_only_op_cpu(bp, ac, yi, yv, im_info, lvl)
            boxes.append(bfs[0])
            stats.append(bfs[1])
        ...
        stats = self._run_reduce_sum_op_cpu(stats)
        stats = stats[0]

        # cls_boxes = []
        # for cls_id in range(num_classes-1):
        #     cfs = self._run_class_based_boxes_only_op_cpu(stats, boxes, cls_id)
        #     cls_boxes.append(cfs[0])
        cls_boxes = self._run_class_based_boxes_combined_op_cpu(stats, boxes, num_classes-1)
        ...
        pim = self._run_reduce_boxes_only_op_cpu(agg_nms_boxes)
        rois = pim[0]
        
        stats = pim[1]
        final = self._run_distribute_op_cpu(rois, stats, 5)
        crois = final[0]
        frois = final[1]
