...
file_prefix = 'anet_detection_vg_fc6_feat'
...
for i in range(num_split):
    ...
    flat_label_proposals = label_proposals[:, :, proposal_per_frm, 6].reshape(N, -1, 6)

    frm_idx = range(proposal_per_frm)
    tile_frm_idx = np.tile(frm_idx.reshape(1,proposal_per_frm,1), (N,1,num_frm)).reshape(N,proposal_per_frm*num_frm, 1)
    ...
    dets_num.append(np.sum(num_proposals, axis=1))
    nms_num.append(np.sum(num_nms, axis=1))

dets_labels_lst = np.stack(dets_labels_lst)
dets_num_lst = np.stack(dets_num_lst)
nms_num_lst = np.stack(nms_num_lst)

print(dets_labels_lst.shape, dets_num_lst.shape, nms_num_lst.shape)
assert(np.mean(dets_num_lst) == np.mean(nms_num_lst))
print('On average, {} out of {} frames have proposals'.format(np.mean(dets_num_lst), num_frm))
